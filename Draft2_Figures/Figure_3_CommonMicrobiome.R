...
complex <- ComplexUpset::upset(shared_df, Tissue_and_Background, name = "Tissue and Background Common Genus",
                               max_size = 100, min_degree=2,width_ratio=0.1, intersections = the_intersections,base_annotations=list(
                                 'Intersection size'=intersection_size( text = list(fontface = 2),
                                   text_colors=c(
                                     on_background='black', on_bar='white'
                                   )
                                 )
                                 + annotate(geom='text', x=1, y=25,label='All Underground', color = 'white', angle = 90, size = 6, fontface = 2)
                                 + annotate(geom='text', x=2, y=25,label='Inbred Underground', color = 'royalblue3', angle = 90, size = 6, fontface = 2)
                                 + annotate(geom='text', x=3, y=25,label='Hybrid Underground', color = 'firebrick', angle = 90, size = 6, fontface = 2)
                                 + annotate(geom='text', x=4, y=25,label='Rhizosphere', color = 'purple', angle = 90, size = 6, fontface = 2)
                                 + annotate(geom='text', x=5, y=25,label='Root', color = 'tan3', angle = 90, size = 6, fontface = 2)
                                 + annotate(geom='text', x=6, y=25,label='Stalk', color = 'olivedrab', angle = 90, size = 6, fontface = 2)),
                               queries = list(
                                 upset_query(
